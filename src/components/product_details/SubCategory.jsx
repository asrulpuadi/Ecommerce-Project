...
class SubCategory extends Component {
    render() {
        const MyList = this.props.propsSubCategoryData
        const CategorySelected = this.props.propsCategory
        const subCategorySelected = this.props.propsSubCategory

        const subCategoryView = MyList.map((dataParam, keyParam) => {
                ...
                <Container className="text-center">
                    <div className='breadbody'>
                        <Breadcrumb>
                            ...
                            <Breadcrumb.Item>
                                <Link to={"/product-subcategory/" + CategorySelected + "/" + subCategorySelected}>{subCategorySelected}</Link>
                            </Breadcrumb.Item>
                        </Breadcrumb>
                    </div>
                    <div className="section-title text-center mb-40 mt-2">
                        <h2>{CategorySelected} - {subCategorySelected}</h2>
                    </div>
                    <Row>
                        {subCategoryView}
                    </Row>
